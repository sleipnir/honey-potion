...
#include "LinkedList.skel.h"

// xdp_md includes
#include <net/if.h>
#include <linux/if_link.h>
#include <signal.h>
void output(struct LinkedList_bpf* skel, uint time, bool all);
void choose_output(struct LinkedList_bpf* skel, bool all);
void output_opt(struct LinkedList_bpf* skel);
void output_all(struct LinkedList_bpf* skel);
...
int main(int argc, char **argv) {
  ...
  struct LinkedList_bpf *skel;
  int err;

  skel = LinkedList_bpf__open();
  if(!skel){
  ...
  err = LinkedList_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    LinkedList_bpf__destroy(skel);
    return -err;
  }


  err = LinkedList_bpf__attach(skel);
if(err){
...
void output_opt(struct LinkedList_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
