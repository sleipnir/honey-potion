...
#include <signal.h>
void output(struct Cond_bpf* skel, uint time, bool all);
void choose_output(struct Cond_bpf* skel, bool all);
void output_opt(struct Cond_bpf* skel);
void output_all(struct Cond_bpf* skel);
...
void output_opt(struct Cond_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
