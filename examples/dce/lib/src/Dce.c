...
#include <signal.h>
void output(struct Dce_bpf* skel, uint time, bool all);
void choose_output(struct Dce_bpf* skel, bool all);
void output_opt(struct Dce_bpf* skel);
void output_all(struct Dce_bpf* skel);
...
int main(int argc, char **argv) {
  ...
  err = Dce_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    Dce_bpf__destroy(skel);
    return -err;
  }


  err = Dce_bpf__attach(skel);
if(err){
...
void output_opt(struct Dce_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
