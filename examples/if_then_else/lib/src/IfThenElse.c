...
#include "IfThenElse.skel.h"

// xdp_md includes
#include <net/if.h>
#include <linux/if_link.h>
#include <signal.h>
void output(struct IfThenElse_bpf* skel, uint time, bool all);
void choose_output(struct IfThenElse_bpf* skel, bool all);
void output_opt(struct IfThenElse_bpf* skel);
void output_all(struct IfThenElse_bpf* skel);
...
int main(int argc, char **argv) {
  ...
  struct IfThenElse_bpf *skel;
  int err;

  skel = IfThenElse_bpf__open();
  if(!skel){
  ...
  err = IfThenElse_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    IfThenElse_bpf__destroy(skel);
    return -err;
  }


  err = IfThenElse_bpf__attach(skel);
if(err){
...
void output_opt(struct IfThenElse_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
