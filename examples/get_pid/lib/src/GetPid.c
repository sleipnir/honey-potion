...
#include <signal.h>
void output(struct GetPid_bpf* skel, uint time, bool all);
void choose_output(struct GetPid_bpf* skel, bool all);
void output_opt(struct GetPid_bpf* skel);
void output_all(struct GetPid_bpf* skel);
...
void output_opt(struct GetPid_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
