...
#include "ListExample.skel.h"

// xdp_md includes
#include <net/if.h>
#include <linux/if_link.h>
#include <signal.h>
void output(struct ListExample_bpf* skel, uint time, bool all);
void choose_output(struct ListExample_bpf* skel, bool all);
void output_opt(struct ListExample_bpf* skel);
void output_all(struct ListExample_bpf* skel);
...
int main(int argc, char **argv) {
  ...
  struct ListExample_bpf *skel;
  int err;

  skel = ListExample_bpf__open();
  if(!skel){
  ...
  err = ListExample_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    ListExample_bpf__destroy(skel);
    return -err;
  }


  err = ListExample_bpf__attach(skel);
if(err){
...
void output_opt(struct ListExample_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
