...
#include "TupleExample.skel.h"

// xdp_md includes
#include <net/if.h>
#include <linux/if_link.h>
#include <signal.h>
void output(struct TupleExample_bpf* skel, uint time, bool all);
void choose_output(struct TupleExample_bpf* skel, bool all);
void output_opt(struct TupleExample_bpf* skel);
void output_all(struct TupleExample_bpf* skel);
...
int main(int argc, char **argv) {
  ...
  struct TupleExample_bpf *skel;
  int err;

  skel = TupleExample_bpf__open();
  if(!skel){
  ...
  err = TupleExample_bpf__load(skel);
  if(err){
    fprintf(stderr, "Failed loading or verification of BPF skeleton.\n");
    TupleExample_bpf__destroy(skel);
    return -err;
  }


  err = TupleExample_bpf__attach(skel);
if(err){
...
void output_opt(struct TupleExample_bpf* skel){    int maxPoints = 3;
    int points = 0;
    while(1) {
        fflush(0);
